...
(function () {
    ...
    function _getOwner(modelName, model, defaultValue, parents, modelOwners, modelParenting, callback) {
        var path = getSearchOwnerPath(modelName, modelOwners, modelParenting);

        __log('path', path);
        //First level is trying to get owner id from request.

        __log('parents in getOwner', parents);

        var hasIds = false;
        var runOverParents = function(){
            forEach(parents, function(parent){
                var next = this.async();
                getIds(parent, path, model, function(err, ids){
                    hasIds = !err&&ids.length>0;
                    if(hasIds){
                        callback(err, ids);
                    }
                    next(!hasIds);
            ...
            getIds(defaultValue, path, model, function(err, ids){
                var hasIds = !err&&ids.length>0;
                if(hasIds){
                    callback(err, ids);
                }else{
                    runOverParents();
