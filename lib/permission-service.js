...
var allPromise = require('./all-promise');
...
PermissionService.prototype.disabled = false;
...
var _evaluateEndpoint = function ($this, req, res, next, callback) {
    ...
    __log('_evaluateEndpoint, restGuard:', policy);
    if (policy) {
        var getUserIds = allPromise.promisify($this.getUserIdCollectionFn());
        var getResource = policy.getResource ? allPromise.promisify(policy.getResource) : null;
        var getOwner = policy.getResourceOwner ? allPromise.promisify(policy.getResourceOwner) : null;
        var getParents = policy.getResourceParents ? allPromise.promisify(policy.getResourceParents) : null;
        ...
        getUserIds(req).then(function (ids) {
            ...
            return getOwner ? getOwner(dataResource, req) : null;
        }).then(function (owner) {
            _owner = owner;
            __log('owner found', _owner);
...
PermissionService.prototype.evaluateEndpoint = function (req, res, next) {
    var fn = allPromise.promisify(_evaluateEndpoint);
    var promise = fn(this, req, res, next);
